...
from core.loss import JointsMSELoss, FundamentalLoss
...
def main():
    ...
    criterion2 = torch.nn.MSELoss(reduction='mean').cuda()

    criterion_dict['mse_weights'] = criterion
    criterion_dict['mse'] = criterion2
    if config.LOSS.USE_FUNDAMENTAL_LOSS:
        criterion3 = FundamentalLoss(config)
        criterion_dict['fundamental'] = criterion3

    # Data loading code
    normalize = transforms.Normalize(
        mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    valid_dataset = eval('dataset.' + config.DATASET.TEST_DATASET)(
        config, 'train', True,  # training set, is_trainin=True
        transforms.Compose([
            transforms.ToTensor(),
            normalize,
        ]),
    ...
    validate(config, valid_loader, valid_dataset, model, criterion_dict,
