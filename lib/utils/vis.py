...
def save_all_preds(pose2d_gt, preds, detected, names, source, save_dir):
    ...
    for img_idx in range(len(pose2d_gt)):
        ...
        image_file = os.path.join(root_path, 'data', source, 'images.zip@', 'images',
                          image_name)
        ...
        for joint_idx in range(len(label)):
            img = cv2.circle(org_img, (int(label[joint_idx][0]), int(label[joint_idx][1])), 5,
                    red, -1)
            img = cv2.drawMarker(img, (int(pred[joint_idx][0]), int(pred[joint_idx][1])),
                green if detected_one_image[joint_idx] else blue, cv2.MARKER_CROSS, 10)
