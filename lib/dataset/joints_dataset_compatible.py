...
class JointsDatasetCompatible(Dataset):
    ...
    def __init__(self, cfg, subset, is_train, transform=None):
        ...
        self.actual_joints = {}
        self.u2a_mapping = {}
        self.flip_pairs = self.get_flip_paris()

        self.color_jitter = tsf.Compose([
            tsf.ToPILImage(),
            tsf.ColorJitter(brightness=(0.7,3.0), contrast=(0.5,2.0), 
                saturation=(0.5, 2.0), hue=0.2),
            ]) if cfg.DATASET.COLOR_JITTER else False
    ...
    def generate_heatmap(self, joints, joints_vis, source):
        ...
        # only when no pseudo label
        if source == 'h36m' and not self.pseudo_label:
