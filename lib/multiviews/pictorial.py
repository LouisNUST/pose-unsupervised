...
def infer(unary, pairwise, body, config):
    ...
    root_idx = body.root_idx  # here, root idx changed to root idx of body
    ...
    for node in skeleton_sorted_by_level:
        # energy = []
        children_state = []
        # requires unary order is the same as order defined in HumanBody object
        ...
        if len(node['children']) == 0:
            ...
            for child in children:
                ...
                if scipy.sparse.issparse(pairwise_mat):
...
def rpsm(cams, heatmaps, boxes, grid_center, limb_length, pairwise_constraint,
    ...
    # heatmaps is orderred as union_dataset ascent order
